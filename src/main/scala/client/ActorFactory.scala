...
class ActorFactory {
   ...
   */
  def createActor(token: String, role: String)(implicit actorSystem: ActorSystem): (Actor, String) = role match {
    case "patient" =>
      val patientID = PatientID(UUID.randomUUID().toString)
      Tuple2(new PatientActor(patientID, token, new PatientGUI(patientID.value, token, actorSystem)), s"patient$patientID")
    case "surgeon" =>
